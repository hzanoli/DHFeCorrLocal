...
def feature_dict(df,name):
    ...
    #check all the ones that start with thoses names
    ...
    #creates a dict with the only the independets and dependents for this particle
...
def plot_histogram(df, bin_values, ax, title=None, scale_x='linear', **kwargs):
    ...
    The axes is mandatory. kwargs are opassed to the set_xscale and set_xscale only."""
...
def hist_with_errors(variable, bins_n='auto', ax = 0, norm =False, return_values = False):
    ...
    try:
        ...
        warnings.warn("Automatic bin_edges failed for variable " + str(variable)+". Replacing with 100", RuntimeWarning)
...
def plot_inv_mass(df,base_name, part_name, pt):
    """Old version (might not work out of the box) used to plot invariant mass.
    In the current implementation of plot_cuts, the invariant mass is treated properly,
    and it is plotted separetaly for particle and antiparticles. This function can help if you want to plot if for both at the same time."""
...
def plot_cuts_part_antipart(before, after, cuts, name_qa, plot_hist=False):
    ...
    col_dict_part = feature_dict(before,particle_name)
    col_dict_anti = feature_dict(before,particle_name+'bar')
    
    after_part = after[after['IsSeleced'+particle_name] == True]
    after_anti = after[after['IsSeleced'+particle_name+'bar'] == True]
    ...
    for col_v, i in zip(col.keys(),range(len(col.keys())) ):
        ...
        try:
            ...
            if (float(selection_pt.loc[col_v]) < min(range_x)):
                line = ax[1].vlines(min(range_x)+1.05(range_x[1]-range_x[0]),min(range_y),1.05*y_max, colors=['blue'],linestyles='solid')
            else:  
                line = ax[1].vlines(float(selection_pt.loc[col_v]),min(range_y),1.05*y_max,
            ...
            try: 
                ...
                if  (float(selection_pt.loc[col_v]) < min(range_x)):
                    warnings.warn('The minimum bin value for col: '+str(col_v)+(' is less than the x value.It will not be shown'),RuntimeWarning)
                else:
